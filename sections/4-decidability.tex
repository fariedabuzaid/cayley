...
Another important ingredient of our proof is to construct a ``small'' $r$-equivalent word from a given word $w$. This is routine since it can be achieved by a simple automata-theoretic approach.
...
\begin{proof}[Proof sketch]
	Construct a first-order formula $\phi$ that characterizes the $r$-type of $v$. From $\phi$ compute an automaton $\calA_\phi$ with $L(\calA_\phi) = \{ w\in\Gamma^\ast \mid w\equiv_r v \}$. One easily shows via induction on $r$ that the size of the automaton $\calA$ is at most
