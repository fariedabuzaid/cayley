...
For $p,q\in \Qx$ let $p\Delta q = (\rd{p}, \wrt{p})\Delta (\rd{q}, \wrt{q})$ and we call $|p\Delta q|$ the ($\Delta$-)distance of $p$ and $q$. \todo{Neighborhood = $\Delta$-Neighborhood}

Let us first give an intuitive description of our decidability proof. We follow a classical proof strategy due to Ferrante and Rackoff \cite{FerR79}. More precisely we show that for every
...
for some fixed primitive recursive function $f$.
...
$r$-neighborhood of each element $p\in \Qx$ is finite for every fixed radius $r$, we can use the above observation to implement an effective decision procedure for the theory of $\calC$.   
